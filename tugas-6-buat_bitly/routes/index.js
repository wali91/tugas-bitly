...
module.exports = function(app, passport) {
        ...
        function(token, done) {
          connection.query(
            ...
            function(err, user) {
              ...
              connection.query(
                "UPDATE users SET reset_password_token= ?,reset_password_expires= DATE_ADD(?, INTERVAL +1 DAY) WHERE email = ?",
                [EditUserMysql.token, EditUserMysql.expires, req.body.email],
                function(err, rows) {
                  done(err, token, user);
                }
              );
  ...
  app.get("/reset/:token", function(req, res) {
    ...
    connection.query(
      "SELECT * FROM users WHERE reset_password_token = ? and reset_password_expires >= DATE_ADD(?, INTERVAL +0 DAY)",
      [EditUserMysql.token, EditUserMysql.expires],
      function(err, user) {
        if (user.length > 0) {
            ...
            function(err, user) {
              ...
              if (req.body.password === req.body.confirm) {
                ...
                connection.query(
                  ...
                  [EditUserMysql.password, req.params.token],
                  function(err, user) {
                    done(err);
                  }
                );
